...
(define (helper loc)
       (cond
         [(empty? loc) empty]
         [(empty? (scan (first loc))) null]
         [(equal? (token-kind (first (scan (first loc)))) 'dotword)
          (printbit (token-lexeme (second (scan (first loc)))))
          ];(helper (rest loc))]
         [else (fprintf (current-error-port) "ERROR\n")]))

(define (printbit num)
  (let [(word empty)]
    (set! word (bitwise-ior (arithmetic-shift num 0) #x00000000))
